...
?>  
    <body>
 <?php    
        if(isset($_POST['Submit'])){  // Stops the page from reloading        

        $gameArray = MaintainState(); // Sets the board back the to submitted vaules        
        $errors = SolveGame();        // Check the answers submitted       
    }else{         
        ...
        /**
         * Puts both the gameArray into the session to be retrived.
         */
        $_SESSION["GameState"] = &$gameArray;
            
    }
    ?>
        <form action="<?php echo $_SERVER['PHP_SELF'] ;?>" method="post" name="Sudoku" class="gameForm" > 
            <div id="gameboard" >
                <!--Row 1-->
                <span id="row_1"> 
                ...
                <span id="row_9"> 
                    ...
                    <input type="submit" value="Check Your Answers" id="Submit" name="Submit"></input>
                    <input type="Reset" value="Reset Board" id="Reset" name="Reset"></input>
                    <input type="submit" value="New Puzzle" id="Reset" name="new_puzzle"></input>
                    <span id="errors"><p> <?php echo $errors ?></p></span>               
        </form>                
            </div>      
